...
function getStatusValue(name: string): any {
  ...
  const statusTable: StatusTable = {
    ...
    status_editor_encoding: () => {
      ...
      if (editor) {
        ...
        const encoding = vscode.workspace.getConfiguration("files", documentUri).get<string>("encoding");
    ...
    status_git_branch: () => {
      ...
      return branchName || "#no-git-branch";
    },
  ...
  if (name in statusTable) {
    return statusTable[name]();
  }

  // If the provided name is not in the table return the string '#undefined'
  return "#undefined";
}
